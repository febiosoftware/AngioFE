	...
	if (pmat != nullptr) 
	{
		for (int i = 0; i < d.Elements(); i++)
		{
			...
			if (se)	
			{
				...
				for (int j = 0; j < nint; j++)
				{
					FEMaterialPoint * mp = se->GetMaterialPoint(j);
	...
	for (int i = 0; i < NE; i++)
	{
		...
		if (se) 
		{
			...
			if (angio_element) 
			{
				...
				//! Get the interpolated SPD from the shape function-weighted Average Structure Tensor
				mat3ds SPD_int = weightedAverageStructureTensor(SPDs_nodes, H, nint);
...
int GetLocalSoluteID(FEMaterial* pm, int nsol)
{
	...
	if (ptm) 
	{
		...
		if (ptm->m_pSolute[0]->GetSoluteID() == nsol) 
			nsid = 0;
		else if (ptm->m_pSolute[1]->GetSoluteID() == nsol) 
			nsid = 1;
	}
