...
mat3ds FEAngioMaterial::Stress(FEMaterialPoint& mp)
{
	...
	if (angioPt)
	{
		...
		FEElasticMaterialPoint& vessel_elastic = *vp->ExtractData<FEElasticMaterialPoint>();
		...
		FEElasticMaterialPoint& matrix_elastic = *matp->ExtractData<FEElasticMaterialPoint>();
		...
		s = angioPt->m_as + (vess_w * vessel_elastic.m_s) + (matr_w * matrix_elastic.m_s);
	}
	return s;
}
...
double FEAngioMaterial::StrainEnergyDensity(FEMaterialPoint& mp)
{
	...
	if (angioPt)
	{
		...
		double vess_w = angioPt->vessel_weight;
		double matr_w = angioPt->matrix_weight;
		double vess_sed = common_properties->vessel_material->ExtractProperty<FEElasticMaterial>()->StrainEnergyDensity(*vp);
		double matr_sed = matrix_material->ExtractProperty<FEElasticMaterial>()->StrainEnergyDensity(*matp);
...
double FEAngioMaterial::GetSegmentVelocity(	AngioElement* angio_element, vec3d local_pos, double time_shift)
{
	auto mesh = m_pangio->GetMesh();
	double const vel_init = 1.0;
	double vel = velocity_manager->ApplyModifiers(vel_init, local_pos, angio_element, time_shift, mesh);
	return vel;
}
...
double FEAngioMaterial::GetMin_dt(AngioElement* angio_elem, FEMesh* mesh)
{
	...
	for (int i = 0; i < se->GaussPoints(); i++)
	{
		...
		double vel = angio_elem->_angio_mat->GetSegmentVelocity(angio_elem, pos, 0.0);
	...
	switch (se->Type())
	{
		...
		case FE_Element_Type::FE_TET10GL11:
			min_side_length_so_far /= 2.0;
		default:
			break;
	}

	double min_dt = (min_side_length_so_far * 0.5 * dt_safety_multiplier) / max_grow_velocity;
	return min_dt;
}
...
void FEAngioMaterial::ProtoGrowSegments(AngioElement* angio_elem, double end_time, int buffer_index)
{
	assert(angio_elem);
	double min_scale_factor = m_pangio->min_scale_factor;
	double bounds_tolerance = m_pangio->bounds_tolerance;
...
void FEAngioMaterial::GrowthInElement(double end_time, Tip* active_tip, int source_index, int buffer_index)
{
	...
	double grow_vel = this->GetSegmentVelocity(angio_element, local_pos, cur_seg_time);
	...
	vec3d psc_dir = psc_manager->ApplyModifiers(vec3d(1.0, 0.0, 0.0), angio_element, local_pos, active_tip->GetDirection(mesh), mesh);
	// determine contributions from local stimuli
	vec3d pdd_dir = pdd_manager->ApplyModifiers(vec3d(1.0, 0.0, 0.0), angio_element, local_pos, active_tip->initial_fragment_id, buffer_index, continue_growth, psc_dir, alpha, mesh, m_pangio);
	if (!continue_growth)
		return;
	...
	bool proj_success = m_pangio->ScaleFactorToProjectToNaturalCoordinates(se, nat_dir, local_pos, factor);
	...
	double possible_grow_length = m_pangio->InElementLength(se, local_pos, possible_natc);
	...
	if ((possible_grow_length > grow_len) && proj_success)
	{
		...
		vec3d local_growth = (nat_dir * (factor * (grow_len / possible_grow_length)));
		vec3d real_natc = local_pos + local_growth;
		...
		assert(nse->Type() 	== FE_Element_Type::FE_TET4G4 
							? (local_pos.x + local_pos.y + local_pos.z) < 1.01 
							: true);
		assert(nse->Type() == FE_Element_Type::FE_TET4G1 
							? (local_pos.x + local_pos.y + local_pos.z) < 1.01 
							: true);
		...
		bool vess_thresh_exceeded = angio_element->vessel_weight > angio_element->_angio_mat->thresh_vess_weight;
		if (angio_element->_angio_mat->branch_policy && !vess_thresh_exceeded)
		{
			//SL: could be a function of seg.
			...
			if (seg->LengthSinceBranch >= l2b) 
			{
				...
				vec3d real_natc_growth = (nat_dir * (factor * rel * (grow_len / possible_grow_length)));
				vec3d real_natc_b = local_pos + real_natc_growth;
	...
	else if (proj_success)
	{
		...
		if (angio_element->_angio_mat->branch_policy && !vess_thresh_exceeded) 
		{
			...
			if (seg->LengthSinceBranch >= l2b) 
			{
				double rel = (l2b - seg->parent->LengthSinceBranch) / (seg->LengthSinceBranch - seg->parent->LengthSinceBranch);
				double branch_time = active_tip->time + (rel * dt);
				vec3d real_natc_growth = (nat_dir * (rel * (possible_grow_length / grow_len)));
				vec3d real_natc_b = local_pos + real_natc_growth;
				double final_time = branch_time;
				angio_element->_angio_mat->branch_policy->AddBranchTipEFD(angio_element, real_natc_b, seg, branch_time, next->initial_fragment_id, buffer_index, mesh);
				seg->LengthSinceBranch = seg->LengthSinceBranch - l2b;
		...
		for (int i = 0; i < angio_element->adjacency_list.size(); i++)
		{
			...
			if (ang_elem)
			{
				...
				FESolidDomain* sd = dynamic_cast<FESolidDomain*>(ase->GetMeshPartition());
				if (sd)
				{
					double r[3];
					bool natc_proj = m_pangio->ProjectToElement(*ase, pos, mesh, r);
		...
		if (possible_locations.size())
		{
			...
			int index = SelectNextTip(possible_locations, possible_local_coordinates, next, dt, buffer_index);
			if (index != -1)
			{
				...
				ang_elem->active_tips[next_buffer_index].at(ang_elem).push_back(next);
			}
		}
		...
		else
		{
			...
			// If bounce, then bounce off face. Else grow slightly off the face.
			vec3d reflected_dir = face_norm * ((nat_dir * face_norm) / face_norm.norm2());
			vec3d plane_projection;
			if (m_pangio->bounce) 	
				plane_projection = nat_dir - reflected_dir * 2.0;
			else 
				plane_projection = nat_dir - reflected_dir * 1.1;
...
void FEAngioMaterial::ProtoGrowthInElement(double end_time, Tip* active_tip, int source_index, int buffer_index)
{
	////////// Initialization //////////
	...
	// get the growth direction
	double alpha = proto_cm_manager->ApplyModifiers(dt, angio_element, local_pos, mesh);
	// determine contributions from previous segments
	vec3d proto_psc_dir = proto_psc_manager->ApplyModifiers(vec3d(1.0, 0.0, 0.0), angio_element, local_pos, active_tip->GetDirection(mesh), mesh);
	// determine contributions from local stimuli
	vec3d proto_pdd_dir = proto_pdd_manager->ApplyModifiers(vec3d(1.0, 0.0, 0.0), angio_element, local_pos, active_tip->initial_fragment_id, buffer_index, continue_growth, proto_psc_dir, alpha, mesh, m_pangio);
	// get the new direction in global coordinates
	...
	else if (proj_success)
	{
		...
		for (int i = 0; i < angio_element->adjacency_list.size(); i++)
		{
			...
			if (ang_elem && (proto_cross || same_mat))
			{
				...
				if (sd)
				{
					...
					bool natc_proj = m_pangio->ProjectToElement(*ase, pos, mesh, r);
					if (natc_proj && m_pangio->IsInBounds(ase, r, bounds_tolerance))
					{
						possible_locations.push_back(angio_element->adjacency_list[i]);
						possible_local_coordinates.push_back(FEAngio::clamp_natc(angio_element->adjacency_list[i]->_elem->Type(), vec3d(r[0], r[1], r[2])));
					}
		...
		// if there are no adjacent elements to grow into then currently this just keeps the tip active.
		else
		{
			...
			if (m_pangio->bounce) 
				plane_projection = nat_dir -  reflected_dir * 2.0;
			//SL: The proto bounce value is 1.0 (exactly along the plane) but the growth has 1.0...
			else 
				plane_projection = nat_dir - reflected_dir * 1.0;
			plane_projection.unit();
			next->direction = plane_projection;
			next->use_direction = true;
			AngioElement* ang_elem = next->angio_element;
			ang_elem->active_tips[next_buffer_index].at(next->angio_element).push_back(next);
			ang_elem->recent_segments.push_back(seg);
			ang_elem->reference_frame_segment_length += seg->RefLength(mesh);
		}
	}
...
int FEAngioMaterial::SelectNextTip(std::vector<AngioElement*>& possible_locations, std::vector<vec3d>& possible_local_coordinates, Tip* tip, double dt, int buffer)
{
	auto mesh = m_pangio->GetMesh();
	double min_scale_factor = m_pangio->min_scale_factor;
	...
	for (int i = 0; i < possible_locations.size(); i++)
	{
		...
		nat_dir.unit();
		vec3d global_pos = this->m_pangio->Position(se, local_pos);
...
void FEAngioMaterial::PostGrowthUpdate(AngioElement* angio_elem, int buffer_index)
{
...
FEMaterialPointData* FEAngioMaterial::CreateMaterialPointData()
{
	...
	FEMaterialPointData* vdata = common_properties->vessel_material->CreateMaterialPointData();
