...
double DensFAContributionMix::ApplyModifiers(double dt, AngioElement* angio_element, vec3d local_pos, FEMesh* mesh)
{
	...
	for (int i = 0; i < nint; i++)
	{
		...
		double dens_mp = angio_mp->ref_ecm_density * (1.0 / elastic_mp->m_J);
		density_at_integration_points.push_back(dens_mp);
	}
	...
	double density_at_point = interp.Interpolate(se, density_at_integration_points, local_pos, mesh);
	double a = std::min(
				std::max(
					a_min, 
...
{
	//SL: Syntax not intuitive. Rewrite later.
