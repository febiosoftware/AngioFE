...
struct EigComp {
	bool operator()(const std::pair<double, vec3d>& x, std::pair<double,
					vec3d>& y) const 
	{
		if ((x.first) > (y.first))
		{
...
double PSCPDDContributionMix::
	ApplyModifiers(	double dt, AngioElement* angio_element, vec3d local_pos, 
					FEMesh* mesh)
{
		return psc_weight;
}

//! this is supposed to update the psc to a load curve? Probably also will allow
//! it to be updated by fractional anisotropy, concentrations, gradients, etc.
void PSCPDDContributionMix::Update(FEMesh * mesh)
{
	// TODO: Implement.
}

double DensFAContributionMix::
	ApplyModifiers(	double dt, AngioElement* angio_element, vec3d local_pos, 
					FEMesh* mesh)
{
	...
	//! SL: Is this supposed to be over nodes? Should probably be gauss pts
	...
	double density_at_point = 
		interp.Interpolate(	se, density_at_integration_points, local_pos, mesh);
	double a = std::min(
				std::max(
					a_min, 
					a_min * (density_at_point - 2.0)), 
				0.3);
	
	// solve for alpha
	double alpha = a0 + 
		a 
		/ (1.0 + exp(b * (angioFA_int - c)));
	return alpha;
}
...
double ContributionMixManager::
	ApplyModifiers(	double prev, AngioElement* angio_element, vec3d local_pos, 
					FEMesh* mesh)
{
	for (int i = 0; i < cm_modifiers.size(); i++)
	{
		prev = cm_modifiers[i]->
			ApplyModifiers(prev, angio_element, local_pos, mesh);
	}
	return prev;
}

double ProtoPSCPDDContributionMix::
	ApplyModifiers(	double dt, AngioElement* angio_element, vec3d local_pos, 
					FEMesh* mesh)
{
	//! dt is assumed to be 1 for the proto step as it only runs from -1 to 0. 
	//! Initial segments will only grow straight if the dt is used.
...
double ProtoContributionMixManager::
	ApplyModifiers(	double prev, AngioElement* angio_element, vec3d local_pos, 
					FEMesh* mesh)
{
	for (int i = 0; i < proto_cm_modifiers.size(); i++)
	{
		prev = proto_cm_modifiers[i]->
			ApplyModifiers(prev, angio_element, local_pos, mesh);
	}
	return prev;
}