...
vec3d PositionDependentDirectionManager::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int buffer, bool& continue_growth, 
					vec3d& tip_dir, double& alpha, FEMesh* mesh, FEAngio* pangio)
{
	for (int i = 0; i < pdd_modifiers.size(); i++)
	{
		prev = pdd_modifiers[i]->
			ApplyModifiers(	prev, angio_element, local_pos, initial_fragment_id , 
							buffer , alpha, continue_growth , tip_dir , mesh, pangio);
	}
	return prev;
}
...
vec3d LagrangePStrainPDD::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha, 
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, 
		FEAngio* pangio) 
{
	...
	least_strain.unit();
	vec3d next_dir = angio_element->_angio_mat->mix_method->
		ApplyMixAxis(prev, least_strain, beta);
	return next_dir;
}

vec3d ECMDensityGradientPDD::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha, 
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, 
					FEAngio* pangio)
{
	...
	for (int i = 0; i < se->GaussPoints(); i++)
	{
		...
		density_at_integration_points.
			push_back(angio_mp->ref_ecm_density * (1.0 / elastic_mp->m_J));
	}
	...
	if (gradnorm > threshold)
	{
		...
		vec3d new_dir = angio_element->_angio_mat->mix_method->
			ApplyMix(prev, perpendicularToGradient, contribution);
		if (prev* perpendicularToGradient < 0) 
		{ 
			new_dir = -new_dir; 
		}
		return new_dir;
					...
					FEAngio* pangio)
{
	...
	for (int i = 0; i < se->GaussPoints(); i++)
	{
		...
		repulse_at_integration_points.
			push_back(angio_mp->repulse_value * (1.0 / elastic_mp->m_J));
	}
...
vec3d ConcentrationGradientPDD::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha, 
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, 
					FEAngio* pangio)
{
	...
	vec3d grad 
		= pangio->gradient(se, concentration_at_integration_points, local_pos);
	double gradnorm = grad.norm();
	if (gradnorm > threshold)
	{
		if (alpha_override)
		...
		vec3d new_dir = angio_element->_angio_mat->mix_method->
			ApplyMix(prev, grad, contribution);
		return new_dir;
	}
	return prev;
}

vec3d FisherConcentrationGradientPDD::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha,	
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, 
					FEAngio* pangio)
...
double AnastamosisPDD::
	distance2(FESolidElement * se, vec3d local_pos, Tip * tip, FEMesh* mesh)
{
	vec3d pos;
	double H[FESolidElement::MAX_NODES];
	se->shape_fnc(H, local_pos.x, local_pos.y, local_pos.z);
					...
					FEAngio* pangio)
{
	...
	Tip* best_tip = FuseWith(angio_element, pangio, mesh, tip_pos, tip_dir, 
							initial_fragment_id, anastamosis_radius);
	if (best_tip)
	{
...
Tip* AnastamosisPDD::
	FuseWith(class AngioElement* angio_element, FEAngio* pangio, 
			class FEMesh* mesh, class vec3d tip_pos, class vec3d tip_dir, 
			int exclude, double radius)
{
	double best_possible_distance;
	auto se = angio_element->_elem;
	Tip * best 
		= BestInElement(angio_element, pangio, mesh, tip_pos,tip_dir, exclude, 
						best_possible_distance);
...
Tip * AnastamosisPDD::
	BestInElement(	AngioElement* angio_element, FEAngio* pangio, FEMesh* mesh, 
					vec3d tip_origin, vec3d tip_dir, int exclude, double& best_distance)
{
...
vec3d ProtoPositionDependentDirectionManager::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int buffer, bool& continue_growth, 
					vec3d& tip_dir, double& alpha, FEMesh* mesh, FEAngio* pangio)
{
...
vec3d ProtoRepulsePDD::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha, 
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, FEAngio* pangio)
{
	std::vector<double> repulse_at_integration_points;
	auto se = angio_element->_elem;
	// find which angio elements have repulse values
					...
					FEAngio* pangio)
{
	...
	if (best_tip)
	{
		...
		if (len < fuse_radius 
			&& fuse_angle  > dir * tip_dir)
		{
			angio_element->anastamoses++;
			continue_growth = false;
		}
		vec3d new_dir = angio_element->_angio_mat->mix_method->
			ApplyMix(prev, dir, proto_contribution);
		if (prev* dir< 0) 
		{ 
			new_dir = -new_dir; 
		}
			...
			int exclude, double radius)
{
	double best_possible_distance;
	auto se = angio_element->_elem;
	Tip * best = BestInElement(	angio_element, pangio, mesh, tip_pos, tip_dir, 
								exclude, best_possible_distance);
	double best_distance = std::numeric_limits<double>::max();
	...
	while (next.size())
	{
		...
		if (cdist <= radius 
			&& cdist <= best_distance)
		{
			Tip * pos_best = BestInElement(cur, pangio, mesh, tip_pos, tip_dir, 
											exclude, best_possible_distance);
			if (pos_best 
				&& best_possible_distance < best_distance)
			{
				best = pos_best;
				best_distance = best_possible_distance;
			}
					...
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, FEAngio* pangio)
{
	...
	vec3d next_dir = angio_element->_angio_mat->mix_method->
		ApplyMixAxis(tip_dir, fiber_direction, alpha);
	return next_dir;
}
...
vec3d ProtoFractionalAnisotropyPDD::
	ApplyModifiers(vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					int initial_fragment_id, int current_buffer, double& alpha, 
					bool& continue_growth, vec3d& tip_dir, FEMesh* mesh, 
					FEAngio* pangio)
