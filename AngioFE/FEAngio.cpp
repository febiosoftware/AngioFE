...
void FEAngio::ProtoGrowSegments()
{
...
void FEAngio::GetActiveFinalTipsInRadius(AngioElement* angio_element, double radius, FEAngio* pangio, std::vector<Tip *> & tips)
{
	...
	while (next.size())
	{
		...
		//! get the minimum distance from the bounds of the element that we're 
		//! calculating tips in the radius of to the iteration's element
...
//! clamping checks if an element is in the prescribed range and if not moves it 
//! to the closest acceptable value in the range. This clamps a position (natc) 
//! to the range given by the natural coordinates for an element type et
vec3d FEAngio::clamp_natc(int et, vec3d natc)
{
	double etx = std::max(
						std::min(
							NaturalCoordinatesUpperBound_r(et), 
							natc.x),
						NaturalCoordinatesLowerBound_r(et));
	double ety = std::max(
						std::min(
							NaturalCoordinatesUpperBound_s(et),
							natc.y),
						NaturalCoordinatesLowerBound_s(et));
	double etz = std::max(
						std::min(
...
FEAngio::FEAngio(FEModel& fem) : 
	ztopi(std::uniform_real_distribution<double>(0.0, PI)), 
	zto2pi(std::uniform_real_distribution<double>(0.0, 2.0 * PI)), 
	n1to1(std::uniform_real_distribution<double>(-1.0, 1.0))
{	
...
double FEAngio::GetDoubleFromDataStore(int record, int elem_id, int item)
{
	...
	double record_val = ds.GetDataRecord(record)->Evaluate(elem_id, item);
	return record_val;
}
...
{
	//! collagen fibers are right handed so the following transformation is 
	//! legal the following will only produce right handed bases for the 
	//! collagen fibers which is molecularly accurate
	...
	double s2 = sin(beta);
	double s3 = sin(gamma);
	//see: https://en.wikipedia.org/wiki/Euler_angles Z1X2Z3 euler angle rotation
	mat3d rv(	(c1 * c3) - (c2 * s1 * s3),
				- (c1 * s3) - (c2 * c3 * s1),
				s1 * s2,
				(c3 * s1) + (c1 * c2 * s3),
				(c1 * c2 * c3) - (s1 * s3),
...
{
	//! force the order of initialization should allow results on different 
	//! platforms to converge 
...
vec3d FEAngio::gradient(FESolidElement * se, std::vector<double> & fn, vec3d pt)
{
	...
	double	Gr[FESolidElement::MAX_NODES], 
			Gs[FESolidElement::MAX_NODES], 
			Gt[FESolidElement::MAX_NODES];
...
static void solve_3x3(double A[3][3], double b[3], double x[3])
{
	...
	double D =	(A[0][0] * A[1][1] * A[2][2]) + (A[0][1] * A[1][2] * A[2][0]) 
				+ (A[1][0] * A[2][1] * A[0][2]) - (A[1][1] * A[2][0] * A[0][2]) 
...
#pragma omp parallel for 
	for (int i = 0; i < angio_elements_size; i++)
	{
		...
		double vessel_radius = angio_element->_angio_mat->vessel_radius;
		double seg_length = angio_element->reference_frame_segment_length;
...
bool FEAngio::ScaleFactorToProjectToNaturalCoordinates(FESolidElement* se, vec3d & dir, vec3d & pt, double & sf) const
{
	...
	vec3d upper_bounds(	FEAngio::NaturalCoordinatesUpperBound_r(se->Type()), 
						FEAngio::NaturalCoordinatesUpperBound_s(se->Type()), 
						FEAngio::NaturalCoordinatesUpperBound_t(se->Type())
						);

	vec3d lower_bounds(	FEAngio::NaturalCoordinatesLowerBound_r(se->Type()), 
	...
	switch (se->Type())
	{
		...
		case FE_Element_Type::FE_TET10GL11:
		{
			...
			/*
			For each face of TET4G4, check to see what scale factor is needed to 
			have the given vector (pt, dir)	reach the plane shared by the face.
			*/
			...
			*/
			...
			{
				temp0 = temp0.inverse();
				sol0 = temp0 * (pt - vec3d(0.0, 1.0, 0.0)); // pt - v0
			}
			...
			mat3d temp1(-dir, vec3d(0.0, 0.0, 1.0), vec3d(1.0, 0.0, 0.0));
			vec3d sol1;
			...
			mat3d temp2(-dir, vec3d(1.0, 0.0, 0.0), vec3d(0.0, 1.0, 0.0));
			vec3d sol2;
			...
			*/
			// relative_v1 = vec3d(0, 0, 1) - vec3d(0, 0, 0); // v1 - v0
			// relative_v2 = vec3d(0, 1, 0) - vec3d(0, 0, 0); // v2 - v0
			mat3d temp3(-dir, vec3d(0.0, 0.0, 1.0), vec3d(0.0, 1.0, 0.0));
			vec3d sol3;
			...
			/*
			Check to make sure that the scale factor is greater than 0 to ensure 
			growth is simulated. Then ensure that the v1 and v2 components are 
			within the bounds of the face's natural coordinate system. Last, to 
			ensure that the solution is within the face, the sum of v1 and v2 
			components must be less than	1.01 (scaled to account for floating 
