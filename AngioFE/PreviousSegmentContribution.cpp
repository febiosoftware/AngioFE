...
vec3d PreviousSegmentContributionManager::ApplyModifiers(vec3d prev, AngioElement* angio_element, vec3d local_pos, vec3d prev_direction, FEMesh* mesh)
{
	...
	{
		prev = psc_modifiers[i]->
			ApplyModifiers(prev, angio_element, local_pos, prev_direction, mesh);
	}
	return prev;
}

vec3d ProtoPreviousSegmentPSC::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					vec3d prev_direction, FEMesh* mesh)
{
	return prev_direction;
...
// Managers
vec3d ProtoPreviousSegmentContributionManager::
	ApplyModifiers(	vec3d prev, AngioElement* angio_element, vec3d local_pos, 
					vec3d prev_direction, FEMesh* mesh)
{
	for (int i = 0; i < proto_psc_modifiers.size(); i++)
	{
		prev = proto_psc_modifiers[i]->
			ApplyModifiers(prev, angio_element, local_pos, prev_direction, mesh);
	}
	return prev;
}
