...
// reflect incident vector where I is the incident vector and N is the normal vector to the surface
...
inline vec3d mix3d(vec3d & x, vec3d & y, double a)
{
	...
	// determine the normal vector to the plane spdnned by x and y
	vec3d normal_vec = x ^ y;
	normal_vec.unit();
	mat3d rot_mat;
	auto nx = normal_vec.x, ny = normal_vec.y, nz = normal_vec.z;
	double cp = cos(phi), sp = sin(phi);
	// assemble the rotation matrix about the normal vector within the plane spdnned by x and y
