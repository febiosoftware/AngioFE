...
vec3d FECell::GetPosition() const
{
...
vec3d FECell::GetRefPosition() const
{
	double arr[FESolidElement::MAX_NODES];
	assert(angio_element);
	auto se = angio_element->_elem;
...
void FECell::PrintCellInfo() const
{
#ifndef NDEBUG
	auto mesh = angio_element->_angio_mat->m_pangio->GetMesh();
	assert(mesh);
...
void FECell::SetLocalPosition(vec3d pos)
{
	assert(angio_element);
	auto se = angio_element->_elem;
	assert(se->Type() 
...
void FECell::InitSpecies()
{
	FEModel* fem = angio_element->_mat->GetFEModel();
	CellSpeciesManager* m_species 
		= angio_element->_angio_mat->cell_species_manager;
	//! assign the properties to each solute. Initialize body loads and add to 
	//! the solutes container.
	if (m_species) 
	{
		for (int i = 0; i < m_species->cell_solute_prop.size(); i++) 
		{
			...
			cell_solute->CellSolutePS->SetPosition(GetPosition());
			...
			{
				fem->AddModelLoad(cell_solute->CellSolutePS);
		...
		for (int i = 0; i < m_species->cell_SBM_prop.size(); i++)
		{
			...
			cell_sbm->CellSBMPS->SetPosition(GetPosition());
			...
			if (cell_sbm->CellSBMPS->Init()) 
			{
				...
				fem->AddModelLoad(cell_sbm->CellSBMPS);
				SBMs.emplace_back(cell_sbm);
			}
		}
		CellReactionManager* m_reaction 
			= angio_element->_angio_mat->cell_reaction_manager;
...
void FECell::UpdateSpecies()
{
	...
	for (int isol = 0; isol < Solutes.size(); isol++)
	{
		...
		{
			Solutes[isol]->CellSolutePS->SetPosition(GetPosition());
		}
	...
	for (int isbm = 0; isbm < SBMs.size(); isbm++)
	{
		...
		if (this->eval_time >= 0.0) 
		{
			SBMs[isbm]->CellSBMPS->SetPosition(this->GetPosition());
		}
		//! Update the body load
	...
	while (t_r > 0.0)
	{
		...
		for (int isol = 0; isol < nsol; isol++) 
		{
			...
			for (int k = 0; k < Reactions.size(); ++k)
			{
				...
				if (Reactions[k]->m_pFwd) 
				{ 
					Reactions[k]->m_pFwd->SetCell(this); 
				}
				if (Reactions[k]->m_pRev) 
				{ 
					Reactions[k]->m_pRev->SetCell(this); 
				}
				//! Get the reaction supply for each reaction that the solute 
				//! is involved in
				double zetahat;
				zetahat = Reactions[k]->ReactionSupply(this);
				// Get the net stoichiometric ratio for each solute
				double v = Reactions[k]->m_v[isol];
				//! Add the product of the stoichiometric ratio with the 
				//! supply for the reaction
				Sol->AddSolhat(v * zetahat);
				//Sol->AddSolhat(v * zetahat * Sol->GetTolScale());
				std::cout	<< "tol scale during eval is " 
							<< Sol->GetTolScale() << endl;
				//! accumulate unless there's no dt. Solutes have the dt 
				//! evaluated elsewhere.
				Sol->CellSolutePS->
					Accumulate(Sol->GetSolPRhat() * max_dt / dt);
		...
		for (int isbm = 0; isbm < nsbm; isbm++) 
		{
			...
			for (int k = 0; k < Reactions.size(); ++k)
			{
				...
				//! Get the reaction supply for each reaction that the SBM is 
				//! involved in
				double zetahat;
				zetahat = Reactions[k]->ReactionSupply(this);
				//! Get the net stoichiometric ratio for each sbm
				double v = Reactions[k]->m_v[nsol + isbm];
				//! Add the product of the stoichiometric ratio with the supply 
				//! for the reaction
				SBM->AddSBMhat(v * zetahat);
				//! need to include dt for SBMs since they are not handled the 
				//! same as solutes.
				SBM->CellSBMPS->Accumulate(SBM->GetSBMPRhat() * max_dt); 
			}
		}
...
double FECell::GetMaxSpeciesDT(double t0, double dt)
{
	//! SL: Currently catches the case where cells are constantly producing 
	//! species. Unsure if this will cause bugs down the line.
	auto mesh = angio_element->_angio_mat->m_pangio->GetMesh();
	assert(mesh);
	int nsbm = this->SBMs.size();
	int nsol = this->Solutes.size();
	...
	for (int isbm = 0; isbm < nsbm; isbm++)
	{
		...
		if (t0 != 0.0)
		{
			newc = 
				SBM->GetInt() 
				+ ((SBM->GetSBMhat() + SBM->GetSBMhatp()) / 2.0) * dt;
			if (newc < 0.0)
			{
				dt_try = std::min(
								(-2.0 * SBM->GetInt()) / (SBM->GetSBMhat() 
									+ SBM->GetSBMhatp()), 
								dt_max);
				if (dt_try > 0.0) 
				{ 
...
void FECell::ProtoUpdateSpecies()
{
	...
	{
		//! Set the new position
		SBMs[isbm]->CellSBMPS->SetPosition(GetPosition());
	}
};